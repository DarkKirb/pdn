...
LDFLAGS := -specs=3dsx.specs $(ASFLAGS) -Wl,--section-start,.text=0x00100000
...
$(dir_out)/$(name).cia: $(dir_out)/$(name).cxi
	makerom -f cia -o $@ -content $<:0:0
$(dir_out)/$(name).cxi: $(dir_build)/$(name).elf
	makerom -f ncch -rsf $(name).rsf -o $@ -elf $<
